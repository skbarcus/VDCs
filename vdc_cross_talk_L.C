...
void vdc_cross_talk_L(int run){

  gROOT->Reset();

  const TString rootfilePath = "/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/rootfiles/";
  ...
  std::ofstream output1 (Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/u1/Cross_Talk_%i_u1.txt",run,run), std::ofstream::out);
  output1<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output2 (Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/u2/Cross_Talk_%i_u2.txt",run,run), std::ofstream::out);
  output2<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output3 (Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/v1/Cross_Talk_%i_v1.txt",run,run), std::ofstream::out);
  output3<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output4 (Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/v2/Cross_Talk_%i_v2.txt",run,run), std::ofstream::out);
  output4<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output5 (Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/Time_%i.txt",run,run), std::ofstream::out);

  TChain* T = new TChain("T");

  T->SetBranchStatus("*",0);               //Turns off all branches (faster). 
  T->SetBranchStatus("L.vdc.u1.dist",1);   //Turns on single branches for u1.
  ...
  T->SetBranchStatus("L.vdc.u2.dist",1);   //Turns on single branches for u2.
  T->SetBranchStatus("L.vdc.u2.time",1);
  T->SetBranchStatus("L.vdc.u2.wire",1);
  T->SetBranchStatus("L.vdc.u2.nclust",1);
  T->SetBranchStatus("L.vdc.u2.clsiz",1);
  T->SetBranchStatus("L.vdc.u2.clpivot",1);
  T->SetBranchStatus("L.vdc.u2.clpos",1);
  T->SetBranchStatus("L.vdc.u2.nhit",1);
  T->SetBranchStatus("L.vdc.v1.dist",1);   //Turns on single branches for v1.
  T->SetBranchStatus("L.vdc.v1.time",1);
  T->SetBranchStatus("L.vdc.v1.wire",1);
  T->SetBranchStatus("L.vdc.v1.nclust",1);
  T->SetBranchStatus("L.vdc.v1.clsiz",1);
  T->SetBranchStatus("L.vdc.v1.clpivot",1);
  T->SetBranchStatus("L.vdc.v1.clpos",1);
  T->SetBranchStatus("L.vdc.v1.nhit",1);
  T->SetBranchStatus("L.vdc.v2.dist",1);   //Turns on single branches for v2.
  T->SetBranchStatus("L.vdc.v2.time",1);
  T->SetBranchStatus("L.vdc.v2.wire",1);
  T->SetBranchStatus("L.vdc.v2.nclust",1);
  T->SetBranchStatus("L.vdc.v2.clsiz",1);
  T->SetBranchStatus("L.vdc.v2.clpivot",1);
  T->SetBranchStatus("L.vdc.v2.clpos",1);
  T->SetBranchStatus("L.vdc.v2.nhit",1);
  ...
  //==finish adding splits rootfiles=====================//
	...
	//cout<<"Plane = "<<plane<<endl;
	T->SetBranchAddress(Form("L.vdc.%s.wire",plane),wire);
	T->SetBranchAddress(Form("L.vdc.%s.time",plane),time);
	T->SetBranchAddress(Form("L.vdc.%s.dist",plane),dist);
	T->SetBranchAddress(Form("L.vdc.%s.nclust",plane),&nclust);
	T->SetBranchAddress(Form("L.vdc.%s.clsiz",plane),clsiz);
	T->SetBranchAddress(Form("L.vdc.%s.clpivot",plane),clpivot);
	T->SetBranchAddress(Form("L.vdc.%s.clpos",plane),clpos);
	T->SetBranchAddress(Form("L.vdc.%s.nhit",plane),&nhit);
	...
	for(Int_t i=0;i<399;i++)       //Loop over VDC wires in a single plane.
	  {
	    ...
	    */
	    ...
	    c1->SaveAs(Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/%s/pictures_png/%i_%s_wire%i_wire%i.png",run,plane,run,plane,wire1,wire2));
	    c1->SaveAs(Form("/lustre/expphy/work/halla/triton/skbarcus/Analysis/VDCs/%i/%s/pictures_C/%i_%s_wire%i_wire%i.C",run,plane,run,plane,wire1,wire2));
