...
void vdc_testr(int run){
  ...
  std::ofstream output1 (Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/u1/Cross_Talk_%i_u1.txt",run,run), std::ofstream::out);
  output1<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output2 (Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/u2/Cross_Talk_%i_u2.txt",run,run), std::ofstream::out);
  output2<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output3 (Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/v1/Cross_Talk_%i_v1.txt",run,run), std::ofstream::out);
  output3<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output4 (Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/v2/Cross_Talk_%i_v2.txt",run,run), std::ofstream::out);
  output4<<"Wires      Total Counts    CT Counts    CT (%)"<<endl;
  std::ofstream output5 (Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/Time_%i.txt",run,run), std::ofstream::out);
  ...
  //==finish adding splits rootfiles=====================//
	...
	for(Int_t i=0;i<10;i++)       //Loop over VDC wires in a single plane.
	  {
	    wire1 = i;
	    wire2 = i+1;
	    
	    TH2D *h1 = new TH2D("h1",Form("Crosstalk Run %i Plane %s",run,plane) , 100, -0.4e-6, 0.4e-6, 100, -0.4e-6, 0.4e-6);
	    ...
	    */
	    ...
	    c1->SaveAs(Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/%s/pictures_png/%i_%s_wire%i_wire%i.png",run,plane,run,plane,wire1,wire2));
	    c1->SaveAs(Form("/home/skbarcus/Tritium/Analysis/VDCs/Cross_Talk_Code/%i/%s/pictures_C/%i_%s_wire%i_wire%i.C",run,plane,run,plane,wire1,wire2));
    ...
    output5<<"CPU time = "<<st->CpuTime()<<"s = "<<st->CpuTime()/60.<<" min   Real time = "<<st->RealTime()<<" s = "<<st->RealTime()/60.<<" min"<<endl;
    output5.close();
}  
