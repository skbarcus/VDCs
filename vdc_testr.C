...
void vdc_testr(int run){
  ...
  const TString rootfilePath = "/adaqfs/home/a-onl/tritium/skbarcus/rootfiles/";
	...
	for(Int_t k=0;k<25;k++)
	  {
		...
		for(Int_t m=0;m<400;m++)
		  {
		    if(wire[m]==wire1 && wire[m+1]==wire2 && TMath::ATan(d_th[k])*180/3.141592653 <55. && TMath::ATan(d_th[k])*180/3.141592653 > 45.)
		      {
